  ...
  setup(__props, { expose: __expose, emit: __emit }) {
    ...
    const addField = (field) => {
      ...
      if (field.propString) {
        if (initialValues.has(field.propString)) {
          ...
          initialValues.set(field.propString, lodashUnified.cloneDeep(field.fieldValue));
        }
      }
    };
    ...
    const setInitialValues = (initModel) => {
      ...
      for (const key of initialValues.keys()) {
        initialValues.set(key, lodashUnified.cloneDeep(objects.getProp(initModel, key).value));
      }
      fields.forEach((field) => {
        if (field.prop) {
          field.setInitialValue(objects.getProp(initModel, field.prop).value);
        }
      });
    };
    const resetFields = (properties = []) => {
      ...
      const propsToCheck = properties.length > 0 ? lodashUnified.castArray(properties).map((p) => shared.isArray(p) ? p.join(".") : p) : [...initialValues.keys()];
      for (const propString of propsToCheck) {
        if (!activePropStrings.has(propString) && initialValues.has(propString)) {
          objects.getProp(props.model, propString).value = lodashUnified.cloneDeep(
